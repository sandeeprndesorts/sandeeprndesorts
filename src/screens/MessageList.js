...
import {GetMatchUserChatList} from '../services/matches.chat.list';
...
export default function MessageList() {
  ...
  const getMatchesList = async () => {
    setLoader(true);
    const responseMatches = await GetMatchUserChatList();
    if (responseMatches.status === 200) {
      setMatchedChatRooms(responseMatches?.data?.matched_chatrooms);
    }
    setLoader(false);
  };
  useEffect(() => {
    ...
    <BaseView
      ...
      {!loader && (
        <View style={{margin: width(5)}}>
          <Text style={{fontSize: fontSize(3.5), justifyContent: 'center'}}>
            People who have liked you
          </Text>
          <FlatList
            data={matchedChatRooms}
            horizontal
            keyExtractor={(item, index) => index}
            renderItem={({item, index}) => {
              return (
                <TouchableOpacity activeOpacity={1} onPress={chatDetail}>
                  <View
                    style={{
                      flex: 1,
                    ...
                    <Image
                      source={{
                        uri:
                          item.user?.profile_pic?.url === null
                            ? NoProfilePhoto
                            : `${mediaBaseUrl} ${item.user?.profile_pic?.url}`,
                      }}
                      style={{height: height(15), width: width(25)}}
                    />
                    <View>
                      <Text
                        style={{
          ...
          <View
            ...
            <Image
              source={require('../assets/icons/searchChat.png')}
              style={{
                width: width(6),
                height: width(6),
                justifyContent: 'center',
