...
import {GetGenders, SignUpStep4} from '../services/CreateAccount.services';
...
export default function SignIn4() {
  ...
  const next = async () => {
    setLoader(true);
    const responseSignUp4 = await SignUpStep4(selected);
    console.log(responseSignUp4, 'signUp4');
    if (responseSignUp4.status === 200) {
      navigation.navigate('SignIn5');
    }
    setLoader(false);
  };
